...
function Bot() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages([...messages, { text: "Sorry, the server is not responding", sender: "bot" }]);
    } finally {
      setInput("");
      setLoading(false);
    }
    ...
    <div className='flex flex-col min-h-screen bg-[#0d0d0d] text-white'>
      <header className="fixed top-0 left-0 w-full border-b border-gray-800 bg-[#0d0d0d] z-10">
        <div className="container mx-auto flex justify-between items-center px-6 py-4">
          <h1 className="text-lg font-bold text-blue-400">ChatBot</h1>
        ...
        <div className="w-full max-w-4xl mx-auto px-4 flex flex-col space-y-3">
          {messages.length === 0 ?
            <div className="text-center text-gray-400 text-lg">
              👋 Hi, I'm <span className="text-blue-500 font-semibold">ChatBot</span>.
            </div> :
            <>
              {messages.map((msg, idx) => (
                <div key={idx} className={`px-4 py-2 rounded-xl max-w-[75%] ${msg.sender === "user" ? "bg-blue-600 self-end" : "bg-gray-800 self-start"}`}>
                  {msg.text}
                </div>
              ))}
              {loading && <div className="bg-gray-700 text-gray-300 px-4 py-2 rounded-xl max-w-[60%] self-start">Bot is typing...</div>}
      ...
      <footer className="fixed bottom-0 left-0 w-full border-t border-gray-800 bg-[#0d0d0d] z-10">
        <div className="max-w-4xl mx-auto flex justify-center px-4 py-3">
          <div className="w-full flex bg-gray-900 rounded-full px-4 py-2 shadow-lg">
            ...
            />
            <button onClick={handleSendMessage} className="bg-blue-600 hover:bg-green-700 px-4 py-1 rounded-full text-white font-medium transition-colors">
              Send
            </button>
          </div>
        </div>
